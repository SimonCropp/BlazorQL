...
- Storage keys are namespaced, configurable per instance via the `StorageNamespace` parameter. The component's default is `blazorql:`; a bundled mount derives its own from the app and the mount (`IdeEndpoint.ForMount` — `blazorql/{IHostEnvironment.ApplicationName}/{mount}:`), because localStorage is scoped per origin and every IDE a host has served would otherwise share every key. The app is load-bearing, not decoration: the mount alone does not separate two services that each take the default mount on the default port. Derive from the mount, never the request path — `MapUnknownPathsToIde` serves one mount from many paths. One `<BlazorQLIde/>` per page remains a documented limitation: the completion and hover providers are registered page-globally and routed by model uri.
